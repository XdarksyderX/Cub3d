...
/*   Updated: 2024/02/28 21:34:23 by migarci2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	ft_cast_ray(mlx_image_t *img, int x, t_game *game)
{
	...
	ft_ray(game, ray_info.angle, &ray_info);
	ray_info.x = x;
	if (ray_info.is_vertical)
		ray_info.hit_ratio = fmod(ray_info.hit[Y], TILE_SIZE)
			/ TILE_SIZE;
	else
		ray_info.hit_ratio = fmod(ray_info.hit[X], TILE_SIZE)
			/ TILE_SIZE;
